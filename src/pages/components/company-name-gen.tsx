...
const INSERT_CONVERSATION_MUTATION = gql`
  mutation PostQuestion($question: String) {
    insert_conversations_one(object: { question: $question }) {
      id
      conversation_id
      question
    }
...
const CompanyNameGenPage = (props: PropsType) => {
  ...
  const [postQuestion, { data, error, loading }] = useMutation(
    INSERT_CONVERSATION_MUTATION
  );
  const subscriptionResult = useSubscription(CONVERSATION_SUBSCRIPTION, {
  ...
  const handleGenerate = async () => {
    ...
    for (let i = 0; i < companyNameQuestionsObj.length; i++) {
      prompt +=
        ", " +
        companyNameQuestionsObj[i]?.question +
        ": " +
        companyNameQuestionsObj[i]?.answer;
    }
    ...
    try {
      ...
      localStorage?.setItem(
        "contextId",
        response?.data?.insert_conversations_one?.id
      );
    } catch (error) {
  ...
  const handleRetry = async () => {
    ...
    try {
      const response = await postQuestion({
        variables: { question: prompt, conversation_id: contextId },
      });
      setContextId(response?.data?.insert_conversations_one?.id);
      localStorage?.setItem(
        "contextId",
      ...
      {subPage == "generation" ? (
            ...
            {currentQuestionIndex === 0 ? (
              <Select
                ...
                onChange={(e: any) => {
                  ...
                  updatedCompanyNameQuestionsObj[currentQuestionIndex].answer =
                    e.value;
              ...
              <>
                <input
                  ...
                  className={`border ${
                    inputError ? "border-red-500" : "border-gray-400"
                  } w-full p-4 border-2 border-black rounded-lg resize-none`}
                  value={inputText}
                  onChange={(e) => {
                    setInputText(e.target.value);
                    let updatedCompanyNameQuestionsObj =
                      companyNameQuestionsObj;
                    updatedCompanyNameQuestionsObj[
        ...
        <>
          <h1
            ...
            style={{
              marginTop: subscriptionResult?.data?.conversations_by_pk?.answer
                ? "60px"
                : "0px",
            }}
          >
            {subscriptionResult?.data?.conversations_by_pk?.answer
              ? "Choose any company name, and if you don't love any, simply hit retry."
          ...
          {subscriptionResult?.loading != undefined &&
            (subscriptionResult?.loading == true ||
          ...
          {subscriptionResult?.loading != undefined &&
            subscriptionResult?.loading != true &&
            subscriptionResult?.data != undefined &&
            subscriptionResult?.data?.conversations_by_pk?.answer != null && (
              <div
