...
const TaglineGenPage = (props: PropsType) => {
  ...
  const [selectedCompanyTagline, setSelectedCompanyTagline] =
    useState<string>("");
  ...
  const handleGenerate = async () => {
    ...
    const example_output_format =
      "company tagline1, company tagline2, company tagline3";
    for (let i = 0; i < taglineQuestionsObj.length; i++) {
      prompt +=
        ", " +
        taglineQuestionsObj[i]?.question +
        ": " +
        taglineQuestionsObj[i]?.answer;
    }
          ...
          <div className="w-full px-4 md:max-w-2xl md:my-12">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                updatedTaglineQuestionObj[currentQuestionIndex].answer =
                  e.target.value;
        ...
        <>
          ...
          >
            {subscriptionResult?.data?.conversations_by_pk?.answer
              ? "Choose any tagline, and if you don't love any, simply hit retry."
              : "Generating tagline..."}
          </h1>
